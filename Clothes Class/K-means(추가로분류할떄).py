...
class MultiCategoryImageClassifier:
    ...
    def _save_progress_batch(self):
        ...
        try:
            with open(self.progress_file, 'w', encoding='utf-8') as f:
                json.dump(progress_data, f, ensure_ascii=False, indent=2)
            logger.info(f"진행상황 일괄 저장 완료: {len(self.completed_categories)}개 카테고리")
        except Exception as e:
            logger.warning(f"진행상황 저장 실패: {e}")
    
    def mark_categories_as_completed(self, category_names):
        """수동으로 카테고리들을 완료 상태로 표시"""
        if isinstance(category_names, str):
            category_names = [category_names]
        
        added_count = 0
        for category_name in category_names:
            if category_name not in self.completed_categories:
                self.completed_categories.add(category_name)
                added_count += 1
                logger.info(f"✅ 완료 처리: {category_name}")
            else:
                logger.info(f"⚠️  이미 완료됨: {category_name}")
        
        # 진행상황 파일 업데이트
        if added_count > 0:
            self._save_progress_batch()
            logger.info(f"📝 {added_count}개 카테고리를 완료 상태로 업데이트했습니다.")
    
    def remove_completed_categories(self, category_names):
        """완료 상태에서 카테고리들을 제거 (다시 처리하고 싶을 때)"""
        if isinstance(category_names, str):
            category_names = [category_names]
        
        removed_count = 0
        for category_name in category_names:
            if category_name in self.completed_categories:
                self.completed_categories.remove(category_name)
                removed_count += 1
                logger.info(f"🔄 완료 상태 해제: {category_name}")
            else:
                logger.info(f"⚠️  완료 목록에 없음: {category_name}")
        
        if removed_count > 0:
            self._save_progress_batch()
            logger.info(f"📝 {removed_count}개 카테고리를 완료 목록에서 제거했습니다.")
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 🎯 이미 분류된 카테고리들을 수동으로 완료 처리
        already_classified = [
            "걸리시_걸리시_001_reg_분류결과",
            "걸리시_걸리시_002_reg_분류결과", 
            "걸리시_걸리시_003_reg_분류결과",
            "고프코어_고프코어_001_reg_분류결과",
            "고프코어_고프코어_002_reg_분류결과", 
            "고프코어_고프코어_003_reg_분류결과",
            "레트로_레트로_001_reg_분류결과", 
            "레트로_레트로_002_reg_분류결과",
            "레트로_레트로_003_reg_분류결과",
            "로맨틱_로맨틱_001_reg_분류결과", 
            "미니멀_001_reg_분류결과",
            "미니멀_002_reg_분류결과", 
            "미니멀_003_reg_분류결과"
        ]
        
        logger.info("🔧 이미 분류된 카테고리들을 완료 상태로 표시합니다...")
        classifier.mark_categories_as_completed(already_classified)
